...
{
    m_detectionActive[kFaceClassLabel] = false;
}
...
EventList ObjectTracker::generateDetectionFinishedEvents(int64_t timestampUs)
{
    ...
    // detect only face
    if(!m_detectionActive[kFaceClassLabel])
        return result; // nothing to finish

    // check if there is any active track for face
    bool hasActiveTrack = false;
    for (const auto& [trackId, track]: m_tracks)
    {
        if (track->status() == Track::Status::active || track->status() == Track::Status::started)
        {
            hasActiveTrack = true;
            break;
        }
    ...
    if(!hasActiveTrack) 
    {
        // generate detection finished event
    ...
    const std::shared_ptr<DetectionInternal>& detection)
{
    ...
    if ((trackStatus == Track::Status::started || trackStatus == Track::Status::active) &&
        !m_detectionActive[kFaceClassLabel])
    {
            events.push_back(std::make_shared<Event>(Event{
